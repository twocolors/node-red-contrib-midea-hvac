...
const SRC = 17;
const APP_KEY = 'ff0cf6f5f0c3471de36341cab3f7a9af'; // 3742e9e5842d4ad59c2db887e12449f9
...
class Midea {
    ...
    return new Promise((resolve, reject) => {
      ...
      this._request({
        ...
        if (error.message.match(/^(3123|3176)/) ) {
          resolve({error: `Command wrong or device (${applianceId}) not reachable`, errorCode: error.message.split(':')[0]});
        }
        reject(error);
      });
    });
  ...
  _request(options) {
    return new Promise((resolve, reject) => {
      ...
      // form
      options.form.appId = APP_ID;
      options.form.format = FORMAT;
      options.form.clientType = CLIENT_TYPE;
      options.form.language = LANGUAGE;
      options.form.src = SRC;
